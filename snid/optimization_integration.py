...
def is_optimization_enabled() -> bool:
    ...
    try:
        from snid.core.integration import get_global_cache
...
def enable_optimization(template_dir: str = "templates", 
    ...
    try:
        from snid.core.integration import enable_optimization as core_enable
        from snid.core.config import SNIDConfig
...
def disable_optimization() -> bool:
    ...
    try:
        from snid.core.integration import disable_optimization as core_disable
        core_disable()
...
def optimize_template_loading(template_dir: str) -> Optional[List[Dict[str, Any]]]:
    ...
    if is_optimization_enabled():
        try:
            from snid.core.integration import get_global_cache, convert_cached_templates_to_legacy_format
            cache = get_global_cache()
            if cache:
